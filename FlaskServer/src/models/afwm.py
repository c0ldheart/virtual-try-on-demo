...
class AFWM(nn.Module):
    ...
    def forward(self, cond_input, image_input, target_cloth_mask):
        ...
        masked_cloth = cond_input * target_cloth_mask
        cond_pyramids = self.cond_FPN(self.cond_features(cond_input)) # maybe use nn.Sequential
        image_pyramids = self.image_FPN(self.image_features(masked_cloth))

        x_warp, last_flow, grid_list,  = self.aflow_net(image_input, image_pyramids, cond_pyramids)
        warped_target_cloth_mask = F.grid_sample(target_cloth_mask, last_flow.permute(0, 2, 3, 1), mode='bilinear', padding_mode='border')
        return x_warp, last_flow, grid_list, warped_target_cloth_mask
